...
void setup_DW1000ServerIRQ(int enable)
{
		GPIO_InitTypeDef GPIO_InitStructure;
    EXTI_InitTypeDef EXTI_InitStructure;
    NVIC_InitTypeDef NVIC_InitStructure;
		...
		{
				// Enable GPIO used as DECA IRQ for interrupt
        GPIO_InitStructure.GPIO_Pin = DECASRVIRQ;
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD;  //IRQ pin should be Pull Down to prevent unnecessary EXT IRQ while DW1000 goes to sleep mode
        //GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
        GPIO_Init(DECASRVIRQ_GPIO, &GPIO_InitStructure);
			
				/* Connect EXTI Line to GPIO Pin */
        GPIO_EXTILineConfig(DECASRVIRQ_EXTI_PORT, DECASRVIRQ_EXTI_PIN);
			
				/* Configure EXTI line */
        EXTI_InitStructure.EXTI_Line = DECASRVIRQ_EXTI;
        EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;
        EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising;  //MP IRQ polarity is high by default
        EXTI_InitStructure.EXTI_LineCmd = ENABLE;
        EXTI_Init(&EXTI_InitStructure);
        ...
        NVIC_InitStructure.NVIC_IRQChannel = DECASRVIRQ_EXTI_IRQn;
