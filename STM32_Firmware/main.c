...
int Anthordistance[ANCHOR_MAX_NUM]= {0,0,0,0};
int Anthordistance_count[ANCHOR_MAX_NUM]= {0,0,0,0};
...
void I2C_Test(void)
{
    ...
    for (i=0; i<256; i++)
    {
        if(I2c_Buf_Read[i] != I2c_Buf_Write[i])
        {
            ...
            printf("ERROR:I2C EEPROM? Write/Read Fail\n\r");
            return;
        }
        printf("0x%02X ", I2c_Buf_Read[i]);
        if(i%16 == 15)
    ...
    printf("I2C(AT24C02) Write/Read pass\n\r");
}
...
int main(void)
{
    ...
    //printf("IIC write pass!\r\n");
    // wait >= 10ms

    //sprintf(dist_str, "IIC: %d", AT24C02_Read(I2C1, 0xA0, 10));//0xA0=> address device; 10=> address byte;
    // USART_puts(dist_str,10);
    // printf("init pass!\r\n");
...
#ifdef ANTHOR_0
            if(rx_buffer[ALL_MSG_SN_IDX]%3 != 0)
                continue;
#endif

#ifdef ANTHOR_1
            if(rx_buffer[ALL_MSG_SN_IDX]%3 != 1)
                continue;
#endif

#ifdef ANTHOR_2
            if(rx_buffer[ALL_MSG_SN_IDX]%3 != 2)
                continue;
#endif

            anthor_index = rx_buffer[ALL_MSG_SN_IDX]%3;
...
static void distance_mange(void)
{
    ...
    sprintf(dist_str, "an0:%3.2fm", (float)Anthordistance[0]/1000/Anthordistance_count[0]);
    OLED_ShowString(0, 2," 		   ");
...
static void compute_angle_send_to_anthor0(int distance1, int distance2,int distance3)
{
    ...
    //location
    {
        ...
        angle_msg[LOCATION_INFO_START_IDX + (len++)] = 0x01;//TAG ID
