...
void extract_data() {
  for (int i = 0; i < 4; i++) {
    ...
    // distance_uwb[i] = modify_distance(distance) > 0 ? modify_distance(distance) : 0;
...
void calc() {
  ...
  position_uwb[0] = 0.5 * (Ainv[0][0] * ATb[0] + Ainv[0][1] * ATb[1]);
  position_uwb[1] = 0.5 * (Ainv[1][0] * ATb[0] + Ainv[1][1] * ATb[1]);

  float rmse = 0.0, dc0 = 0.0, dc1 = 0.0, dc2 = 0.0;
  for (i = 0; i < N_ANCHORS; i++) {
    dc0 = position_uwb[0] - anchor_matrix[i][0];
    dc1 = position_uwb[1] - anchor_matrix[i][1];
...
void display_single(int anchor_id) {
  Serial.println(distance_uwb[anchor_id]);
